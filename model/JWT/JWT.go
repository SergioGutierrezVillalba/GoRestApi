...
func NewJWT(user model.User /*,expiration*/) JWT {
	jwt := JWT{
		Header: NewHeader(),
		Payload: NewPayload(user/*,expiration*/),
	}
	return jwt
}
...
func (j *JWT) DumpDataInsideJWTStruct(){
	myClaims, err := j.GetClaims()
